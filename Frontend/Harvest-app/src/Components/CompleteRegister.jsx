...
function RegisterUser() {
  ...
  // 1. Role Logic (Extracted from URL or default)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // NOTE: Using 'api/v1/register/' instead of 'admin/stats'
      // Most Django Rest Registration serializers expect password1/password2
       const response = await axios.post('http://127.0.0.1:8000/api/v1/register/', {
        email: formData.email,
        password: formData.password,  // MUST be 'password' to match fields
        password2: formData.confirmPassword, // MUST be 'password2' to match Serializer
        role: config.apiValue,
        institution_name: formData.institution_name || "" // Added to prevent 500 errors
      }, { 
      ...
      // Logic to pull specific error messages from Django Rest Framework
      const serverError = err.response?.data;
      let errorMsg = "System failed to provision account.";

      if (serverError) {
        // Handle nested DRF error objects (e.g., { "email": ["This field is required."] })
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 flex items-center justify-center font-sans text-slate-900">
      <div className="max-w-md w-full">
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-slate-400 hover:text-slate-600 mb-8 font-bold text-sm transition-all group">
        ...
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl p-8 md:p-10 relative overflow-hidden">
          {status.success ? (
            <div className="py-12 text-center animate-in fade-in zoom-in duration-500">
              ...
              <p className="text-slate-500 text-sm mt-2 font-medium">Account created. Please log in to continue.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <h1 className="text-3xl font-black tracking-tight text-slate-900 uppercase italic">Register {config.label}</h1>
                ...
                <div className="space-y-2">
                   ...
                   <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 pl-12 pr-4 text-sm font-bold focus:bg-white focus:ring-4 focus:ring-slate-100 transition-all outline-none" 
              ...
              <button 
                ...
                className={`w-full ${config.theme} text-white font-black py-5 rounded-2xl hover:brightness-110 shadow-xl transition-all flex items-center justify-center gap-3 mt-4`}
              >
